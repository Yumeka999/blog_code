...
if __name__ == "__main__":
    ...
    # L2 normlization Fit Train 
    over_fit_model = OverFitModel()
    loss_fun = torch.nn.MSELoss()
    l2_lambda = torch.tensor(10000.)
    ...
    plt.subplot(2,2,2)      
    plt.title("L2 normlization(Train Set), Loss=%.3f" % loss_l2_train) 
    plt.xlabel("x") 
    plt.ylabel("y") 
    plt.scatter(train_x_u, train_y)
    ...
    plt.subplot(2,2,4)      
    plt.title("L2 normlization (Test Set), Loss=%.3f" % loss_l2_test) 
    plt.xlabel("x") 
    plt.ylabel("y") 
    plt.scatter(test_x_u, test_y)
